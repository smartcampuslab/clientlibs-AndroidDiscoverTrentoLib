...
public class EventDetailsFragment extends NotificationsSherlockFragmentDT {
	...
	public void onStart() {
		...
		if (mEvent != null) {
			...
			attendTbtn
					.setOnCheckedChangeListener(new OnCheckedChangeListener() {
						...
						public void onCheckedChanged(CompoundButton buttonView,
							...
							// } else
							{
								new SCAsyncTask<Boolean, Void, LocalEventObject>(
			...
			if (mEvent.getCommunityData() != null
				...
				tv.setText(Utils.conceptToSimpleString(mEvent
	...
	private boolean isCertified(LocalEventObject event) {
		if (event.getCustomData() != null
				&& (Boolean) event.getCustomData().get("certified"))
			return true;
		else
			return false;
		...
		if (this.getView() != null) {
			...
			if (mEvent.getCommunityData() != null) {
				...
				if (cd.getRating() != null && !cd.getRating().isEmpty()) {
					Iterator<Map.Entry<String, Integer>> entries = cd
							.getRating().entrySet().iterator();
					float rate = 0;
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == R.id.submenu_show_related_poi) {
			...
			args.putSerializable(PoiDetailsFragment.ARG_POI_ID, getPOI()
					.getId());
			...
			// } else
			{
				...
				mAlert.setPositiveButton("OK",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
									int which) {
								new SCAsyncTask<LocalEventObject, Void, Boolean>(
										getActivity(),
				...
				alert.show();
//				new SCAsyncTask<LocalEventObject, Void, Boolean>(getActivity(),
//						new EventDeleteProcessor(getActivity()))
//						.execute(getEvent());
				return true;
			}
	...
	private class EventDeleteProcessor extends
		...
		@Override
		public Boolean performAction(LocalEventObject... params)
				throws SecurityException, Exception {
	...
	private class AttendProcessor extends
			AbstractAsyncTaskProcessor<Boolean, LocalEventObject> {
		...
		@Override
		public LocalEventObject performAction(Boolean... params)
				throws SecurityException, Exception {
			attend = params[0];
			if (attend) {
		...
		public void handleResult(LocalEventObject result) {
			...
			if (getSherlockActivity() != null)
				if (mEvent.getAttending() == null
						|| mEvent.getAttending().isEmpty()) {
					Toast.makeText(getSherlockActivity(),
							R.string.not_attend_success, Toast.LENGTH_SHORT)
							.show();
					buttonView
							.setBackgroundResource(R.drawable.ic_btn_monitor_off);
				} else {
					Toast.makeText(getSherlockActivity(),
							R.string.attend_success, Toast.LENGTH_SHORT).show();
					buttonView
							.setBackgroundResource(R.drawable.ic_btn_monitor_on);
				}
		}
